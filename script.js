document.addEventListener("DOMContentLoaded", function () {
  ...
  // Function to check if the second page is in the viewport
  function isInViewport(element) {
    const rect = element.getBoundingClientRect();
    return rect.bottom > 0;
  }

  // Function to handle scrolling and show the second page
  function handleScroll() {
    if (isInViewport(secondPage)) {
      secondPage.classList.add("show-second-page");
      window.removeEventListener("scroll", handleScroll);
    }
  }

  // Function to scroll to the second page with a custom timing function
  function scrollToSecondPage() {
    ...
    // You can adjust the cubic-bezier values for your desired speed
    const timingFunction = "cubic-bezier(0.25, 1, 0.5, 1)";
    scrollOptions.timingFunction = timingFunction;

    if ("scrollBehavior" in document.documentElement.style) {
      // Modern browsers with native smooth scrolling support
      secondPage.scrollIntoView(scrollOptions);
    } else {
  ...
  // Trigger the scroll to the second page with a button click
  const scrollButton = document.querySelector("#scroll-button");
  scrollButton.addEventListener("click", scrollToSecondPage);
});
...
function updateScale() {
  ...
  let scaleValue1 = 1.05 + (currentScroll * parallaxScaling1);
